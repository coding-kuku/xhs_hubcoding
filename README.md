...
仓库目前处于 **规则基线和核心逻辑完成、完整界面尚未接线** 的阶段。

| 模块 | 状态 | 位置 |
|---|---|---|
| 最新开仓动作样机 | 可直接试玩 | `index.html`、`app.js`、`styles.css` |
...
| 完整拍卖场与多页面 UI | 待整合 | 见 `docs/engineering/IMPLEMENTATION_STATUS.md` |
| NPC 长线关系、榜一与随机事件 | 规则方向已确认，待实现 | `docs/mechanics/NPC_AND_RANDOM_EVENTS.md` |

根目录页面是当前视觉与开仓手感样机，不代表完整游戏已经完成。旧原型统一放在 `prototypes/archive/`，不再作为新功能开发入口。
...
- 普通货物每日行情控制在 -30% 至 +20%。藏品和碎片可立即出售，也可入库等待更好的回收渠道。
- 初始仓库 30 格；扩仓按 10 格递增并按自然月收租，避免无限囤货。
